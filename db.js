// Configuración de la base de datos SQLite usando better-sqlite3.
// Aquí se crean las tablas y, si están vacías, se cargan datos de ejemplo.
...
const db = new Database(path.join(dataDir, 'equipos.db'));
...
function sembrarDatos() {
  const insertarEquipo = db.prepare(
    `INSERT INTO equipos (nombre, ciudad, estadio, fundacion, imagen_url)
     VALUES (@nombre, @ciudad, @estadio, @fundacion, @imagen_url)`,
  );
  const insertarJugador = db.prepare(
    `INSERT INTO jugadores (equipo_id, nombre, posicion, dorsal, nacionalidad, imagen_url)
     VALUES (@equipo_id, @nombre, @posicion, @dorsal, @nacionalidad, @imagen_url)`,
  );

  // Todo dentro de una transacción para que sea rápido y atómico.
  const cargar = db.transaction(() => {
    ...
    for (const equipo of equipos) {
      const { jugadores, ...datosEquipo } = equipo;
      const info = insertarEquipo.run(datosEquipo);
      const equipoId = info.lastInsertRowid;
      for (const jugador of jugadores) {
        insertarJugador.run({ equipo_id: equipoId, ...jugador });
      }
    }
  });
